...
# ==========================================
# THANH SIDEBAR TỰ ĐỘNG ĐỒNG BỘ ONLINE VÀ KHO
# ==========================================
st.sidebar.markdown("### ⚙️ CẤU HÌNH ĐỐI SOÁT ONLINE")
st.sidebar.info("Thiết lập Link 1 lần duy nhất ở đây. Bấm nút dưới để hệ thống kéo toàn bộ dữ liệu (Bao gồm Nhập Nguyên Liệu) về chạy cục bộ siêu tốc.")
...
if st.sidebar.button("🚀 LƯU & CẬP NHẬT ĐỒNG LOẠT", use_container_width=True):
    if global_onl_link:
        ...
        with st.spinner("⏳ Đang kéo dữ liệu Doanh thu & Nhập nguyên liệu (Chỉ đợi 1 lần duy nhất)..."):
            # 1. Đồng bộ 3 Tab Doanh Thu Online
            for cn_mac_dinh, def_tab in [("Trường Sa", "Lục_TS"), ("Lê Quang Định", "Lục_LQD"), ("Trần Huy Liệu", "Lục_THL")]:
                ...
                try:
                    ...
                    st.sidebar.error(f"❌ Không tải được Tab '{tab_name}'")
            
            # 2. Đồng bộ Tab Nhập Nguyên Liệu
            try:
                df_nhap = doc_sheet_thong_minh(global_onl_link, "Nhập Nguyên Liệu", ['chi nhánh', 'tổng cộng'], time.time())
                df_nhap.to_csv("temp_nhap_nguyen_lieu.csv", index=False)
            except Exception as e:
                st.sidebar.error(f"❌ Không tìm thấy Tab 'Nhập Nguyên Liệu' trong file Google Sheet này.")
                
        st.sidebar.success("✅ Đã lấy số xong! Dữ liệu đã sẵn sàng ở tất cả các Tab.")
        st.rerun()
    else:
...
# ==========================================
# HÀM LÕI KÉO DỮ LIỆU FOOD COST & NHẬP HÀNG 
# ==========================================
def render_food_cost_tab(cn_mac_dinh, prefix_key, default_tab_sheet):
    ...
    nhap_hang_auto = 0
    if os.path.exists("temp_nhap_nguyen_lieu.csv"):
        try:
            df_nhap = pd.read_csv("temp_nhap_nguyen_lieu.csv")
            ...
            col_tong_nhap = next((c for c in reversed(df_nhap.columns.tolist()) if 'tổng cộng' in str(c).lower()), df_nhap.columns[13] if len(df_nhap.columns) > 13 else df_nhap.columns[-1])
    ...
    with col_dt2:
        ...
        # --- KHÔI PHỤC DROPDOWN CHỌN CỘT CHO DOANH THU ONLINE ---
        dt_online = 0
        ...
        if os.path.exists(file_onl):
            try:
                ...
                # AI tự tìm vị trí của 2 cột quy chuẩn để chọn sẵn
    ...
    if ton_dau_auto > 0 or ton_cuoi_auto > 0 or nhap_hang_auto > 0:
        st.success(f"🤖 Hệ thống đã điền tự động dữ liệu từ Tab 1 và Tab Nhập Nguyên Liệu.")
